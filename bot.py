...
class Database:
    ...
    def add_product(self, name, description, price, category=None, image_url=None, stock=999, is_digital=False, digital_content=None):
        ...
        c = conn.cursor()
        c.execute("INSERT INTO products (name, description, price, category, image_url, stock, is_digital, digital_content) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
    ...
    def get_products(self, category=None, limit=50):
        ...
        c = conn.cursor()
        if category:
            c.execute("SELECT id, name, description, price, category, image_url, stock, is_digital FROM products WHERE category = ? AND stock > 0 ORDER BY id DESC LIMIT ?", (category, limit))
        else:
            c.execute("SELECT id, name, description, price, category, image_url, stock, is_digital FROM products WHERE stock > 0 ORDER BY id DESC LIMIT ?", (limit,))
        products = [{"id": row[0], "name": row[1], "description": row[2], "price": row[3], "category": row[4], "image_url": row[5], "stock": row[6], "is_digital": row[7]} for row in c.fetchall()]
        conn.close()
        return products
    ...
    def get_product(self, product_id):
        conn = self.get_connection()
        c = conn.cursor()
        c.execute("SELECT id, name, description, price, category, image_url, stock, is_digital, digital_content FROM products WHERE id = ?", (product_id,))
        row = c.fetchone()
        conn.close()
        if row:
            return {"id": row[0], "name": row[1], "description": row[2], "price": row[3], "category": row[4], "image_url": row[5], "stock": row[6], "is_digital": row[7], "digital_content": row[8]}
        return None
    ...
    def register_user(self, user_id, username=None, first_name=None, last_name=None):
        conn = self.get_connection()
        c = conn.cursor()
        c.execute("INSERT OR IGNORE INTO users (user_id, username, first_name, last_name) VALUES (?, ?, ?, ?)", (user_id, username, first_name, last_name))
        conn.commit()
        conn.close()
    ...
    def get_cart(self, user_id):
        ...
        """, (user_id,))
        cart = [{"product_id": row[0], "name": row[1], "price": row[2], "quantity": row[3], "is_digital": row[4], "stock": row[5]} for row in c.fetchall()]
        conn.close()
        return cart
    ...
    def create_order(self, user_id, items, total, payment_method, payment_id=None):
        ...
        c.execute("INSERT INTO orders (order_number, user_id, items, total, payment_method, payment_id) VALUES (?, ?, ?, ?, ?, ?)",
    ...
    def get_orders(self, user_id=None, limit=50):
        ...
        c = conn.cursor()
        if user_id:
            c.execute("SELECT id, order_number, items, total, status, created_at FROM orders WHERE user_id = ? ORDER BY created_at DESC LIMIT ?", (user_id, limit))
        else:
            c.execute("SELECT id, order_number, items, total, status, created_at FROM orders ORDER BY created_at DESC LIMIT ?", (limit,))
        orders = [{"id": row[0], "order_number": row[1], "items": json.loads(row[2]), "total": row[3], "status": row[4], "created_at": row[5]} for row in c.fetchall()]
        conn.close()
        return orders
